...
The two diagonals on the diagram are the two spectrums most projects actually move along. The **"ship the real thing" axis**: throwaway script → production codebase with CI. The **"kill it on Friday" axis** runs the other way.
...
**For all of these, the cheapest path is vibe coding with a Tier 2 model.** Use DeepSeek V4, Kimi K2.7, or a local Qwen. The speed wins. The quality is "good enough for code you'll throw out." Keep the expensive models in reserve.
...
**Ron's bottom line:** vibe for exploration, real for shipping, PRD as the switch. Use a coding harness with tests for anything that has to keep working. Use Hermes for internal tools and long-running automations. Tier 2 model for the prototype, Tier 1 for production.
</Callout>
