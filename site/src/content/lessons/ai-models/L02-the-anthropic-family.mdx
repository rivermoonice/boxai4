...
Haiku, Sonnet, and Opus are trained by the same lab, on the same roadmap, called by the same `client.messages.create()` call. You're not picking between "Claude" and "not-Claude." You're picking between three gears inside one engine.
</Callout>
...
Anthropic ships new *versions* of each tier on a roughly quarterly cadence — Sonnet 4.0, 4.5, 5.0; Opus 4.6, 4.7, 4.8. The *tier* (Haiku / Sonnet / Opus) is the speed/cost slot. The *version* is which generation of that tier you're using.

Newer is generally better within a tier. But the tier choice matters more than the version choice.
</Callout>
...
Anthropic has been flagged in 2026 for quietly adjusting usage limits on consumer accounts.

Set a hard monthly spend cap on day one — $20 is a reasonable floor. Check usage every Friday. The cap exists; the warning emails don't always arrive in time.
</Callout>
