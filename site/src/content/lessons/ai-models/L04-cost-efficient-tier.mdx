...
Single-task tests don't scale. In production you route by **tier**: cheap model for chat, mid for hard reasoning, Opus only when cheap fails. [Hermes L06](/lessons/hermes/l06-performance-model-routing-cost/) covers tiered routing. Its `/research` slash command uses Kimi for discovery and escalates to Opus only when synthesis needs it.
...
**Ron's bottom line:** Qwen for long-horizon agents. MiniMax for multimodal and long-context speed.
GLM for UI one-shots. Kimi for the daily-driver sweet spot.
Don't pick by price alone — pick by workload, then check the price.
Revisit the default every quarter.
</Callout>
